...
function TestRow({ name, status, duration, error }: TestRowProps) {
  const statusConfig = {
...
export function ProofOfSuccess() {
  ...
  const prodRobots = robotRuns.find(r => r.env === 'production');
...
[INFO] All tests passed successfully.`}
        ...
        <div>
          ...
          <div className="space-y-2">
            <RobotRun 
              env="staging" 
              status={stagingRobots?.status || 'pending'}
              lastRunAt={stagingRobots?.lastRunAt}
              evidenceUrl={stagingRobots?.evidenceUrl}
            />
            <RobotRun 
              env="canary" 
              status={canaryRobots?.status || 'pending'}
              lastRunAt={canaryRobots?.lastRunAt}
              evidenceUrl={canaryRobots?.evidenceUrl}
            />
            <RobotRun 
              env="production" 
              status={prodRobots?.status || 'pending'}
              lastRunAt={prodRobots?.lastRunAt}
              evidenceUrl={prodRobots?.evidenceUrl}
            />
          </div>
